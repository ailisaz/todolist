<template>
  <div class="body">
    ...
    <div class="demo-date-picker">
      ...
      <el-button @click="showEditDialog" type="danger"
    ...
    <el-table :data="listDate" style="width: 100%" stripe height="300">
      <el-table-column type="expand">
        <template #default="props">
          <div m="1">
            <p m="t-0 b-2">任务内容: {{ props.row.task_content }}</p>
          </div>
        </template>
      </el-table-column>
      <el-table-column label="任务优先级" prop="task_order" />
      <el-table-column label="标题" prop="task_title" />
      <el-table-column label="标记" prop="task_mark" />
      <el-table-column label="番茄数" prop="real_num">
          ...
          <div class="dialog-left">
            ...
            <el-scrollbar class="task_list">
              ...
              >
                ...
                <span
                  >{{ item.task_order }}-{{ item.task_title }}-{{ item.task_mark }}-{{
                    item.pre_num
                  }}-{{ item.real_num }}</span
                >
                ...
                <el-dialog v-model="deleteDialogVisible" title="Warning" width="500" center>
                  <span>确认删除这个任务吗？</span>
                  <template #footer>
                    <div class="dialog-footer">
                      <el-button @click="deleteDialogVisible = false">Cancel</el-button>
                      <el-button class="!ml-0" :plain="true" type="danger" @click="deleteTask(item)"
                        >Confirm</el-button
                      >
                    </div>
                  </template>
...
const displayTime = computed(() => {
  const mins = Math.floor(remainSecond.value / 60)
    .toString()
    .padStart(2, '0')
...
watch(selectTime, (newTime) => {
  if (newTime) getTaskInfo(newTime)
})
...
const deleteTask = async (item: TodoItem) => {
  ...
  const index = listDate.value.findIndex((t) => t.task_id === item.task_id)
  if (index != -1) {
  ...
  if (item.todoID) {
    const ret = await deleteSelectTask({ todoID: item.todoID })
    deleteDialogVisible.value = false
    return `删除成功，todoID：${ret}`
  }
}
...
onMounted(() => {
  ...
  alarmAudio.value.src = '../src/assets/不是因为寂寞才想你.aac'
